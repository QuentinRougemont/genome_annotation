...
#use this script to map IsoSeq read from the same species as your genome
...
minimap2 -ax splice  -t $NCPUS -uf --secondary=no $genome $isoseq > $LR/$name.sam
...
gmst2globalCoords.py -t $LR/cupcake7."$name".gff -p $LR/gmst."$name".out -o $LR/gmst."$name".global.gtf -g $genome
