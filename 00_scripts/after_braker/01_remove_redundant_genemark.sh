...
echo "-----------------------------------------------------------------"
echo "remove redundant GeneMark gene in the CDS and Protein fasta files"
echo "-----------------------------------------------------------------"
...
#awk 'NR == FNR {count[$1]++; next} count[$1]>1' tmp1 tmp1 |grep "GeneMark" |cut -f 9 |cut -d ";" -f 2 |uniq |sed -e 's/transcript_id //' -e 's/"//g' -e 's/ //g' > toremove
